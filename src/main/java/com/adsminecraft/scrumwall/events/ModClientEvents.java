...
@Mod.EventBusSubscriber(modid = ScrumWall.MOD_ID, bus = Mod.EventBusSubscriber.Bus.FORGE, value = Dist.CLIENT)
public class ModClientEvents  {
    ...
    public static void onJumpWithStick(LivingEvent.LivingJumpEvent event) {
        ...
        if (player.getHeldItemMainhand().getItem() == Items.STICK) {
            ScrumWall.LOGGER.info("Player jumped with stick");
    ...
    public static void onCraftingTableOpen(GuiOpenEvent event) {
        if (event.isCancelable()) {
            if (event.getGui() instanceof CraftingScreen) {
                ...
                ScrumWall.LOGGER.info("Player tried to open crafting table");
            }
        }
    }
}
